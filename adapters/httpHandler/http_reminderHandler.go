...
func (h *HttpReminderHandler) AddReminderHandler(c *fiber.Ctx) error {
	...
	err := h.reminderUseCase.AddReminder(uint(noteID), userID, data)
	if err != nil {
	...
	return c.JSON(fiber.Map{"message": "Reminder added successfully"})
}
...
func (h *HttpReminderHandler) UpdateReminderHandler(c *fiber.Ctx) error {
	...
	return c.JSON(fiber.Map{"message": "Reminder updated successfully"})
}
